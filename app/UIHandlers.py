...
class AskMeUIHandlers:
    ...
    def set_cloudinary_config(self, cloudinary_cloud_name,
 ...
 cloudinary_api_key, cloudinary_api_secret):        
    ...
    def get_celebs_response_handler(self, keyword):
        ...
        try:
            ...
            return "", "", "", image_utils.fallback_image_implement(), image_utils.fallback_image_implement()
    ...
    def generate_image_stability_ai_handler(self, name_it, question_prompt):
        if self.stability_api_key:
            stability_api = StabilityAPI(self.stability_api_key)
            output_generated_image=stability_api.text_to_image(name_it, [f"A wallpaper photo of {name_it}, by WLOP"])
    ...
    def generate_image_diffusion_handler(self, name_it):
        if name_it:
            image_utils = ImageUtils()
            try: 
                image_prompt = f"A wallpaper photo of {name_it} by WLOP"
                image_generator = DiffusionImageGenerator()
                output_generated_image = image_generator.generate_image(name_it,image_prompt)
    ...
    def ask_chatgpt(self, prompt, keyword, prompttype):
        ...
        try:        
            ...
            if self.api_key:
                ...
                return self.NO_API_KEY_ERROR, response
        except Exception as err:
            return self.NO_API_KEY_ERROR,""
    ...
    def describe_handler(self, name, prompt, folder_name, input_celeb_real_picture, input_celeb_generated_picture):
        ...
        try:
            ...
            return f"Error {err} in describe_handler", "", "", "", image_utils.fallback_image_implement(), image_utils.fallback_image_implement()
